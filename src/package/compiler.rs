...
use crate::{
    ...
    package::compiler::errors::CompileFileError,
};
use std::{
    collections::HashSet,
    path::PathBuf,
...
impl PackageCompiler {
    ...
    pub fn compile(&self, compiler: &dyn Compiler) -> Result<(), errors::CompilePackageError> {
        ...
        if !compilation_errors.is_empty() {
            return Err(errors::CompilePackageError::CompilationError(
                compilation_errors,
            ));
        }
        if object_files.is_empty() {
            return Err(errors::CompilePackageError::NoFilesToCompile);
        }
        ...
        if !link_command_output.status.success() {
            return Err(errors::CompilePackageError::LinkingError {
